"""
Парсер для сети О'КЕЙ (okeydostavka.ru)
"""
from typing import List
from .base import BaseParser, Product


class OKMarketParser(BaseParser):
    def __init__(self):
        super().__init__("О'КЕЙ")
        self.base_url = "https://www.okeydostavka.ru"
        self.catalog_urls = ["https://www.okeydostavka.ru/catalog/moloko-i-molochnye-produkty/"]
    ...
    def parse(self) -> List[Product]:
        ...
        for url in self.catalog_urls:
            ...
            if not soup:
                continue
            cards = soup.select('.product-card, .goods-item, [class*="product"]')
            print(f"[{self.shop_name}] Найдено элементов: {len(cards)}")
            for card in cards[:50]:
                try:
                    name_el = card.select_one('.name, .title, h3')
                    name = name_el.get_text(strip=True) if name_el else ""
                    if not name or len(name) < 3:
                        continue
                    price_el = card.select_one('.price, .cost-current')
